class Messenger {
	...
	_installListener() {
		...
		this._listener = function (event) {
			...
			if (!event.data || typeof event.data !== 'string' || !this._listener) {
				return;
			}
			...
			if (typeof json.replyId !== 'undefined') {
				...
				that.onMessage(json.message, event.origin, event.source, replyMessage);
			}
		};
	...
	/**
	 * @access public
	 * @description Send a message to an other window
	...
	sendMessage(win, message, origin, options) {
		...
		if (options && options.waitForReply && !this._listener) {
			const that = this;
